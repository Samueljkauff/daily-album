...
export function useSpotifyApi() {
  ...
  async function authBootsrap(): Promise<any> {
    ...
    try {
      ...
      user = await initializeUserSession(CLIENT_ID, code, deviceID, redirect);
      loading = false;
  ...
  async function initializeUserSession(clientId: string, code: string, deviceID: string | null, redirect: string): Promise<User> {
    deviceID = await Storage.get("deviceID");
    if(deviceID === null) {
